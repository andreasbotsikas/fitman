        ...
        try:
            ...
            if results: #bring it from the database
                ...
                for twitter_username in twitter_usernames:
                    twitter_properties+='{"match_phrase_prefix" : { "doc.user_screen_name":"twitter:%s" }},'%twitter_username.replace(" ", "").replace("@", "")
            ...
            for property in properties.keys():
                ...
                number = Counter(re.findall(r, (json.dumps(response)).lower()))
 ...
 #                   number = json.dumps(response).count(phrase)
            ...
            for message in response:
			    ...
			    r2 = re.compile("|".join(r"\b%s\b" % w["name"] for w in category["properties"]),re.I)
		            number2 = Counter(re.findall(r2, (json.dumps(message["_source"]["doc"]["text"])).lower()))
		            if True:
